...
public class SimuladorFIFOSC {
    ...
    private static void executarSimulacaoFIFO_SC() {
        int ponteiroSC = 0;
        ...
        for (int i = 0; i < SIMULATION_STEPS; i++) {
            ...
            if (indiceRam != -1) {
                ...
                while (true) {
                    Pagina paginaCandidata = listaRam.get(ponteiroSC);

                    if (paginaCandidata.R == 0) {
                        if (paginaCandidata.M == 1) {
                            int indiceSwap = paginaCandidata.N;
                            matrizSwap[indiceSwap] = new Pagina(paginaCandidata);
                        ...
                        listaRam.set(ponteiroSC, novaPagina);

                        ponteiroSC = (ponteiroSC + 1) % RAM_SIZE;
                        break;
